...
class InvoiceComponent:
    ...
    def get_all():
        try:
            # Esta consulta une la factura con la persona (cliente) para mostrar el nombre
            sql = """
                SELECT 
                    i.inv_id,
                    i.inv_number,
                ...
                JOIN ceragen.admin_person p ON i.inv_client_id = p.per_id
...
class SchedulingComponent:
    ...
    def get_available_sessions_for_patient(patient_id):
        try:
            # Esta consulta calcula las sesiones totales vs. las consumidas
            # para cada paquete comprado por un paciente.
            sql = """
                SELECT 
                    inv.inv_id,
                    inv.inv_number,
                    prod.pro_id,
                    prod.pro_name,
                    COUNT(sc.sec_id) as total_sessions,
                    COUNT(CASE WHEN sc.ses_consumed = true THEN 1 END) as consumed_sessions,
                    (COUNT(sc.sec_id) - COUNT(CASE WHEN sc.ses_consumed = true THEN 1 END)) as remaining_sessions
                ...
                WHERE inv.inv_patient_id = %s AND inv.inv_state = true
                GROUP BY inv.inv_id, prod.pro_id, prod.pro_name
                HAVING (COUNT(sc.sec_id) - COUNT(CASE WHEN sc.ses_consumed = true THEN 1 END)) > 0;
            """
            record = (patient_id,)
            ...
            return internal_response(True, result['data'], "Paquetes y sesiones del paciente obtenidos exitosamente.")
        except Exception as err:
            message = str(err)
            HandleLogs.write_error(message)
            return internal_response(False, None, message)
